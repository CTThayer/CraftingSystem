...
public class NavMeshInputSourceEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        bool lNewUseStrafingInput = EditorGUILayout.Toggle(new GUIContent("Use Focus Strafe", "Determines if we simulate input as if we're strafing."), mTarget.UseStrafingInput);
        ...
        Transform lNewFocus = EditorGUILayout.ObjectField(new GUIContent("Focus", "When strafing, the object we'll have the Nav Mesh Agent face and rotate towards."), mTarget.Focus, typeof(Transform), true) as Transform;
        if (lNewFocus != mTarget.Focus)
        {
            mIsDirty = true;
            mTarget.Focus = lNewFocus;
        }

        float lNewFocusSpeed = EditorGUILayout.FloatField(new GUIContent("Focus Speed", "Angles per second to rotate the character to face the focus."), mTarget.FocusSpeed);
        if (lNewFocusSpeed != mTarget.FocusSpeed)
        {
            mIsDirty = true;
            mTarget.FocusSpeed = lNewFocusSpeed;
        }

        bool lNewForceFocus = EditorGUILayout.Toggle(new GUIContent("Force Focus", "Determines if we force the rotation even if the character isn't moving."), mTarget.ForceFocus);
        if (lNewForceFocus != mTarget.ForceFocus)
        {
            mIsDirty = true;
            mTarget.ForceFocus = lNewForceFocus;
        }
