...
namespace com.ootii.Actors
{
    ...
    public class ActorController : BaseSystemController, ICharacterController
    {
        ...
        /// </summary>
        protected Vector3 mAccumulatedVelocity = Vector3.zero;
        ...
        private void InternalUpdate(float rDeltaTime, int rUpdateIndex)
        {
            ...
            if (!mIgnoreUseTransform && _UseTransformPosition)
            {
                ...
                mState.Velocity = (mState.Position - mPrevState.Position) / rDeltaTime; // (Time.smoothDeltaTime > 0f ? Time.smoothDeltaTime : Time.deltaTime);
...
//                        }
            ...
            if (lAllowSmoothStep)
            {
                ...
                if (lFinalVerticalMovementDot > EPSILON)
                {
                    if (_StepUpSpeed > 0f)
                    {
                        ...
                        lFinalVerticalMovement = Vector3.MoveTowards(Vector3.zero, lFinalVerticalMovement, Mathf.Max(lFinalLateralMovement.magnitude, rDeltaTime * 1f) * _StepUpSpeed);
                ...
                else if (lFinalVerticalMovementDot < -EPSILON)
                {
                    if (_StepDownSpeed > 0f)
                    {
                        ...
                        if (lStepDown)
                        {
                            ...
                            lFinalVerticalMovement = Vector3.MoveTowards(Vector3.zero, lFinalVerticalMovement, Mathf.Max(lFinalLateralMovement.magnitude, rDeltaTime * 1f) * _StepDownSpeed);
                            lFinalMovement = lFinalLateralMovement + lFinalVerticalMovement;
                        }
                    }
                }
            ...
            if (mOnPreControllerMove != null)
            {
                ...
                // Determine the per second velocity
                mState.Velocity = (lFinalPosition - mPrevState.Position) / rDeltaTime; // (Time.smoothDeltaTime > 0f ? Time.smoothDeltaTime : Time.deltaTime);
            ...
            if (_ShowDebug)
            {
                Utilities.Debug.Log.FileScreenWrite(string.Format("isG:{0} gDist:{1:f3} gAng:{2:f3} gFwdAng:{3:f3} spd:{4:f3}", mState.IsGrounded, lGroundDistance, mState.GroundSurfaceAngle, mState.GroundSurfaceForwardAngle, (lFinalMovement.magnitude / rDeltaTime)), 1);
        ...
        protected bool ProcessGrounding(Vector3 rActorPosition, Vector3 rOffset, Vector3 rActorUp, Vector3 rWorldUp, float rGroundRadius, out RaycastHit rGroundHitInfo)
        {
            ...
            if (!lIsGrounded)
            {
                ...
                // essentially slide off the edge. The collision detection part will cause that to happen.
                //
