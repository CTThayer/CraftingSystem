...
namespace com.ootii.Setup
{
    ...
    public static class LayersSetupHelper
    {
        ...
        /// <summary>
        /// Caches the default Layer Definition Set
        /// </summary>
        public static LayerDefinitionSet DefaultLayerDefinitions
        {
            get
            {
                if (_defaultLayerDefinitionSet == null)
                {
                    _defaultLayerDefinitionSet = LoadDefaultLayerDefinitionSet();
                }

                return _defaultLayerDefinitionSet;
            }
        }
        ...
        private static LayerDefinitionSet _defaultLayerDefinitionSet;
        ...
        public static bool AreDefaultLayersSet()
        {
            ...
            // First, try to use the layers defined in the default LayerDefinitions asset
            if (DefaultLayerDefinitions != null)
            {
                var lDefaultLayers = DefaultLayerDefinitions.LayerDefinitions.OrderBy(x => x.Index);
                foreach (var lDefinition in lDefaultLayers)
                {
                    if (lDefinition.Index > 31) { continue; }
                    if (!LayerExistsAt(lDefinition.Name, lDefinition.Index))
                    {
                        return false;
                    }
                }
            ...
            // If that fails to load, fall back on hard-coded default values
            for (int i = DefaultLayers.Climb; i <= DefaultLayers.PhysicsObjects; i++)
            {
        ...
        public static LayerDefinitionSet LoadDefaultLayerDefinitionSet()
        {
            ...
            if (lDefinitionSet == null)
            {
                ...
                // Add the default layer definitions
        ...
        {
            LayerDefinitionSet lLayers = DefaultLayerDefinitions;
        ...
        /// <summary>
        /// Checks if any of the default layers are in use
        /// </summary>
        /// <param name="rLayersInUse">(Output) Display-ready list of layers in use</param>
        /// <returns></returns>
        public static bool AreDefaultLayersInUse(out string rLayersInUse)
        {
            ...
            foreach (var lDefinition in lDefaultLayers)
            {
                ...
                SerializedProperty lLayer = LayersProperty.GetArrayElementAtIndex(lDefinition.Index);
                if (string.IsNullOrEmpty(lLayer.stringValue)) { continue; }
                if (lLayer.stringValue == lDefinition.Name) { continue; }

                lInUse = true;
                lBuilder.AppendLine(string.Format("Layer [{0}] = {1} will be overwritten with {2}", 
                    lDefinition.Index, lLayer.stringValue, lDefinition.Name));
            }
           
            rLayersInUse = lBuilder.ToString();
            return lInUse;
        }
