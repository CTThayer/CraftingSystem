...
namespace com.ootii.Input
{
    ...
    public class NavMeshInputSource : UnityInputSource
    {
        ...
        /// <summary>
        /// When strafing, determines the focus we'll use to rotate the character.
        /// </summary>
        ...
        /// <summary>
        /// Determines the angles per second we'll use to rotate.
        /// </summary>
        public float _FocusSpeed = 120f;
        public float FocusSpeed
        {
            get { return _FocusSpeed; }
            set { _FocusSpeed = value; }
        }

        /// <summary>
        /// Determines if we won't quit 'naving' until we finish focusing.
        /// </summary>
        public bool _ForceFocus = false;
        public bool ForceFocus
        {
            get { return _ForceFocus; }
            set { _ForceFocus = value; }
        }
        ...
        public void ClearTarget()
        {
            ...
            mIsInSlowDistance = false;
            mIsTargetSet = false;
            mAgentDestination = Vector3Ext.Null;
        ...
        protected void OnControllerLateUpdate(ICharacterController rController, float rDeltaTime, int rUpdateIndex)
        {
            ...
            if (_UseStrafingInput && _Focus != null)
            {
                if (mMovementX != 0f || mMovementY != 0f)
                {
                    mViewX = GetMoveFocusYaw(); 
                    mInputFromAvatarAngle = mViewX;
                    mInputFromCameraAngle = mViewX;
                }
                else if (_ForceFocus)
                {
                    Quaternion lRotation = Quaternion.Euler(0f, GetIdleFocusYaw(), 0f);
        ...
        protected void SimulateStrafeInput()
        {
            ...
            // Force the camera angle to the specified avatar angle
            if (mMotionController._CameraTransform == null)
            {
                mInputFromCameraAngle = mInputFromAvatarAngle;
            }
            // Determine the camera angle based on movement
            else
            {
        ...
        /// <summary>
        /// Used to set the angle for rotation when trying to focus on something
        /// </summary>
        protected virtual float GetIdleFocusYaw()
        {
            float lTargetFocusAngle = 0;

            if (_Focus != null)
            {
                Vector3 lFocusDirection = Vector3.Normalize(_Focus.position - _Transform.position);
                lTargetFocusAngle = Vector3Ext.SignedAngle(_Transform.forward, lFocusDirection);

                float lTargetFocusAngleAbs = Mathf.Abs(lTargetFocusAngle);

                if (lTargetFocusAngleAbs <= EPSILON)
                {
                    lTargetFocusAngleAbs = 0f;
                }
                else if (lTargetFocusAngleAbs >= _FocusSpeed * Time.deltaTime)
                {
                    lTargetFocusAngleAbs = _FocusSpeed * Time.deltaTime;
                }

                lTargetFocusAngle = Mathf.Sign(lTargetFocusAngle) * lTargetFocusAngleAbs;
            }

            return lTargetFocusAngle;
        }

        /// <summary>
        /// Used to set the angle for rotation when trying to focus on something
        /// </summary>
        protected virtual float GetMoveFocusYaw()
        {
            float lTargetFocusAngle = 0;
            ...
            if (_Focus != null)
            {
                ...
                float lTargetFocusAngleAbs = Mathf.Abs(lTargetFocusAngle);
                lTargetFocusAngleAbs = Mathf.Min(lTargetFocusAngleAbs * Time.deltaTime, _FocusSpeed * Time.deltaTime);
