...
public class ItemPartSegment : MonoBehaviour
{
    ...
    [SerializeField] private float DurabilityModifier;
     ...
     */
    [SerializeField] private ItemPartSegment[] ConnectedSegments;
    ...
    void Start()
    {
        ...
        Debug.Assert(ConnectionPoints.Length == ConnectionIDs.Length);
    ...
    public ItemPartSegment[] GetConnectedSegments() { return ConnectedSegments; }
     ...
     */
    public void AddConnectedSegmentAt(int index, ItemPartSegment SegToAdd)
    {
     ...
     * returns false.
     */
    public bool RemoveConnectedSegment(ItemPartSegment segToRemove)
    {
        for (int i = 0; i < ConnectedSegments.Length; i++)
        {
     ...
     * retrieve them.
     */
    public ItemPartSegment[] ClearConnections()
    {
        List<ItemPartSegment> disconnectedSegments = new List<ItemPartSegment>();
     ...
     */
    public void GetAllDownstreamSegments(ref List<ItemPartSegment> segments)
    {
        segments.Add(this);
        for (int i = 1; i < ConnectedSegments.Length; i++)
        {
     ...
     * Returns the next SegmentConnectPoint from this segment that is open 
     * (doesn't have an established connection) OR it returns null if there are
     * no SegmentConnectionPoints that fit these criteria.
     */
    ...
    public void CheckForContact()
    {
        ...
        if (hitColliders != null && hitColliders.Length > 0)
        {
            ...
            for (int i = 0; i < hitColliders.Length; i++)
            {
                ...
                ItemPartSegment otherSegment = hitColliders[i].gameObject.GetComponent<ItemPartSegment>();
